...
- `VEHICLE_ARCHETYPES` (`vehicles/archetypes.ts`) : « méta-voitures » fictives, une par couple (catégorie × énergie) ayant ≥ 3 modèles réels. Chaque spec = **médiane** du segment, calculée en priorité sur les modèles **grand public** (les marques premium de `PREMIUM_BRANDS` sont écartées tant qu'il reste ≥ 3 modèles grand public, sinon on retombe sur le bucket complet — cas des segments réellement premium type berline élec). Seuls les modèles récents (`releaseYear >= ARCHETYPE_MIN_RELEASE_YEAR`, 2022) entrent dans la médiane : un archétype représente le marché **actuel**, pas la moyenne incluant l'occasion ancienne. `buildArchetypes()` est pure. Marquées `isArchetype: true`. Servent à comparer des usages sans dépendre d'un modèle ; l'âge reste géré par `purchaseCondition`. Chaque archétype emprunte la **photo du modèle réel le plus représentatif** de son segment (le plus proche du prix médian, avec image), attribution d'origine conservée — aucun véhicule n'est sans photo.
...
- **Images véhicules** : photos CC (Wikimedia Commons) self-hostées dans `apps/web/public/vehicles/`, mappées dans `vehicles/images.generated.ts` (`VEHICLE_IMAGES`) et fusionnées dans les presets par `vehicles/index.ts`. Générées par `scripts/fetch-vehicle-images.mjs` (à relancer pour ajouter des modèles ; ne récupère que des licences CC/domaine public ; fallback fr→en Wikipédia, préfixe `en:`/`fr:` pour forcer une langue, extension dérivée du fichier source). Chaque image porte un `imageCredit` (auteur + licence + lien) — **attribution obligatoire** affichée par `VehiclePreview.vue` (fallback `VehicleSilhouette.vue` si pas d'image). ⚠️ Ne jamais ingérer d'images/données de sources protégées (L'Argus…) sans accord Direction + licence.
